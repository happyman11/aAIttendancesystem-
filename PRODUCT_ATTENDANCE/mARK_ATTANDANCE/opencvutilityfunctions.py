...
def capture_frames():
    ...
    while True:
        ...
        if not flag:
            ...
            Id,Accuracy=recognise_person(gray_image[y:y+h,x:x+w])
            
            if Accuracy<70:
            
                #msg="Attandance Marked: "+str(Id)
                ...
                #cv2.putText(img_resized,msg, notification_area,font, 1, (0,255,0), 1)
                #cv2.putText(mesh_returned_frame,msg, notification_area,font, 1, (0,255,0), 1)
                #cv2.imshow('Attandance window',img_resized)
                cv2.imshow('Attandance window',mesh_returned_frame)
                
            else:
            
                msg="Unrecognized Person"
                mesh_returned_frame=draw_mesh(img_resized) 
                #cv2.putText(img_resized,msg, notification_area,font, 1, (0,0,255), 2)
                cv2.rectangle( mesh_returned_frame, (cordinates["start_x"], cordinates["start_y"]), (cordinates["end_x"],cordinates["end_y"]), (0, 0, 255), 2)
                cv2.putText(mesh_returned_frame,msg, notification_area,font, 1, (0,0,255), 2)
                #cv2.imshow('Attandance window',img_resized)
                cv2.imshow('Attandance window',mesh_returned_frame)
